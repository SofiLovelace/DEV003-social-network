...
function _errorWithCustomMessage(auth2, code, message2) {
...
function modalDeletePost(messageError) {
...
const dataBase = wh(app);
function createID(use) {
  const idGenerator = Math.random().toString(30).substring(2);
  return use + idGenerator;
}
function createUserDoc(user, fullName, photo) {
  return sf(rh(dataBase, "usuarios", user.uid), {
    id: user.uid,
    correo: user.email,
    nombre: fullName,
    foto: photo
...
function createPost(userId, nombre, postContent) {
  const postId = createID("post");
  console.log(postId);
  const today = new Date();
  return uf(sh(dataBase, "publicaciones"), {
    postId,
    userId,
    nombre,
...
function deletePost(idPost) {
  of(rh(dataBase, "publicaciones", idPost));
}
function getUserPosts(userId) {
  const ref = sh(dataBase, "publicaciones");
  const q2 = dl(
    ref,
    Il("date", "desc")
  );
...
function updatePost(postId, newFields) {
  return rf(rh(dataBase, "publicaciones", postId), newFields);
}
const getUserFromFirestore = (userId) => {
  const ref = rh(dataBase, "usuarios", userId);
  return Jl(ref);
};
...
function counterLike(userUid, docPost, idButton) {
  const postLikes = rh(dataBase, "publicaciones", docPost.uid);
  const ref = sh(dataBase, "publicaciones");
  const arrLike = dl(ref, _l("postId", "==", docPost.postId));
  const getData = tf(arrLike);
  let dataLikes = "";
  let cantLikes;
  getData.then((result) => {
  ...
  const q2 = dl(sh(dataBase, "publicaciones"), _l("postId", "==", docPost.postId));
  cf(q2, (querySnapshot) => {
...
function updateComments(userUid, docPost, idButton) {
  const q2 = dl(sh(dataBase, "publicaciones"), _l("postId", "==", docPost.postId));
  cf(q2, (querySnapshot) => {
    querySnapshot.forEach((d) => {
      console.log("===============================>", d.data());
...
function removeComment(post, commentID) {
  ...
  rf(rh(dataBase, "publicaciones", post.uid), {
    comentarios: arrayUpdateComments
  });
}
function paintComments(post, idButton) {
  ...
  postComments.forEach((comentario) => {
    ...
    buttonDeletePost.addEventListener("click", () => {
      commentsContainer.appendChild(modalDeletePost("\xBFEst\xE1s seguro de eliminar este post?"));
...
function comments(post, containerRender, postID) {
  const user = informationUser();
...
function paintLikes(numberLikes, idButton) {
...
function dialogEditPost(idPost, container, spanPost) {
  ...
  const dialog2 = Dialog("Editar Post", formDialog);
...
function showPost(container) {
  ...
  getUserPosts(user.uid).then((result) => {
    ...
    arrayPosts.forEach((doc) => {
      ...
      buttonEditPost.appendChild(editIcon);
      buttonDeletePost.addEventListener("click", () => {
        container.appendChild(modalDeletePost("\xBFEst\xE1s seguro de eliminar este post?"));
        const buttonCancel = document.getElementById("cancel");
        const windowModalDelete = document.getElementById("windowModalDelete");
        buttonCancel.addEventListener("click", () => {
...
const Home = () => {
  ...
  getUserFromFirestore(user.uid).then((result) => {
    ...
    postInput.type = "text";
    postInput.placeholder = "\xBFQu\xE9 vas a compartir?";
    ...
    signOutBtn.title = "Cerrar sesi\xF3n";
...
const Register = () => {
  ...
  passwordConfirm.placeholder = "Confirmar contrase\xF1a";
  ...
  form.addEventListener("submit", (e) => {
    ...
    const passwordValue = password.value;
    const nameValue = name2.value;
    const lastnameValue = lastname.value;
    const fullName = `${nameValue} ${lastnameValue}`;
    let message2;
    if (passwordValue !== passwordConfirmValue) {
      message2 = "Las contrase\xF1as no coinciden";
      container.appendChild(modalError(message2));
      ...
      registerNewUser(emailValue, passwordValue).then((result) => {
        ...
        if (errorCode) {
          if (errorCode === "auth/email-already-in-use") {
            message2 = "Ya hay un usuario registrado con el correo";
          } else if (errorCode === "auth/internal-error" || errorCode === "auth/invalid-email") {
            message2 = "Ingrese un correo valido";
          } else if (errorCode === "auth/weak-password") {
            message2 = "La contrase\xF1a debe tener minimo 6 caracteres";
          } else if (errorCode === "invalid-argument") {
            message2 = "Algo sali\xF3 mal";
          }
          container.appendChild(modalError(message2));
          const windowModal = document.getElementById("textErrorModal");
          windowModal.addEventListener("click", () => {
  ...
  loginBtn.textContent = "Regresar al inicio de sesi\xF3n";
  ...
  form.append(name2, lastname, email, password, passwordConfirm, registerBtn, loginBtn);
...
const Profile = () => {
  ...
  form.append(name2, lastname, email, profileBtn, loginBtn);
...
function validationloginWithMail(email, password) {
  if (email && password) {
    const promise = emailLogin(email, password);
    return promise.then(() => {
      const message2 = "Usuario logueado";
      console.log(message2);
      return message2;
    }).catch((error) => {
      let message2 = "Algo sali\xF3 mal";
      if (error.code === "auth/wrong-password") {
        message2 = "Contrase\xF1a incorrecta";
      }
      if (error.code === "auth/user-not-found") {
        message2 = "Correo no registrado";
      }
      if (error.code === "auth/invalid-email") {
        message2 = "Correo invalido";
      }
      const mainContainer = document.querySelector(".mainContainer");
      mainContainer.appendChild(modalError(message2));
      const windowModal = document.getElementById("textErrorModal");
      windowModal.addEventListener("click", () => {
...
function validationLoginWithGoogle() {
